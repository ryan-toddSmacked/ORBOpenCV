...
static std::string s_image1_mask;
static std::string s_image2_mask;
...
//========================================================
// Static homography parameters

static double s_ransac_reproj_thresh;
static int s_ransac_max_ite;
static double s_ransac_confidence;
...
/**
 * @brief Read the masks from disk
 *
 * @param mask1 Reference to the mask for image 1
 * @param mask2 Reference to the mask for image 2
 */
inline void readMasks(cv::cuda::GpuMat& mask1, cv::cuda::GpuMat& mask2);
...
void parseArgs(int argc, char** argv)
{
    ...
    po::options_description desc_homography("Homography options", 160, 80);
    ...
    desc_gpu.add_options()
        ...
        ("nlevels", po::value<int>(&s_nlevels)->default_value(3), "The number of pyramid levels.")
        ...
        ("image1-mask", po::value<std::string>(&s_image1_mask), "Mask for image 1")
        ("image2-mask", po::value<std::string>(&s_image2_mask), "Mask for image 2");
    ...
    desc_homography.add_options()
        ("ransac-reproj-thresh", po::value<double>(&s_ransac_reproj_thresh)->default_value(3.0), "Random sample consensus threshold")
        ("ransac-max-ite", po::value<int>(&s_ransac_max_ite)->default_value(2000), "Random sample consensus maximum iterations")
        ("ransac-confidence", po::value<double>(&s_ransac_confidence)->default_value(0.995), "Random sample consensus confidence");

    desc.add(desc_gpu).add(desc_smart_search).add(desc_homography);
...
void singleProcess_quiet(const cv::cuda::GpuMat& img1, const cv::cuda::GpuMat& img2, cv::Mat& homography)
{
...

    {
...

        {
            ...
            readMasks(mask1, mask2);

            s_gpu_orb->detectAndComputeAsync(img1, mask1, keypoints1, descriptors1, false, stream1);
            s_gpu_orb->detectAndComputeAsync(img2, mask2, keypoints2, descriptors2, false, stream2);
    ...
    homography = cv::findHomography(points1, points2, cv::RANSAC, s_ransac_reproj_thresh, cv::noArray(), s_ransac_max_ite, s_ransac_confidence);
}
...
void smartSearch_quiet(const cv::cuda::GpuMat& img1, const cv::cuda::GpuMat& img2, cv::Mat& homography)
{
    ...
    do {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            homographies[i] = std::move(cv::findHomography(points1, points2, cv::RANSAC, s_ransac_reproj_thresh, cv::noArray(), s_ransac_max_ite, s_ransac_confidence));
        }
...
inline void readMasks(cv::cuda::GpuMat& mask1, cv::cuda::GpuMat& mask2)
{
    cv::Mat mask1_cpu, mask2_cpu;

    if (!s_image1_mask.empty())
        try
        {
            cv::imread(s_image1_mask, cv::IMREAD_GRAYSCALE).convertTo(mask1_cpu, CV_8UC1);
            mask1.upload(mask1_cpu);
        }
    ...
    if (!s_image2_mask.empty())
        try
        {
            cv::imread(s_image2_mask, cv::IMREAD_GRAYSCALE).convertTo(mask2_cpu, CV_8UC1);
            mask2.upload(mask2_cpu);
        }
        catch(const std::exception& e)
        {
